<template>
    <div class="whole">
      ...
      <div class="fish-body clear-fix">
        ...
        <div class="each-leafs clear-fix" v-if="sortByDate.length" v-for="(branch,index) in sortByDate" :key="index" :my="testYear(sortByDate)" :class="_addClass(index)" :ref="index">
  ...
  export default {
    ...
    methods:{
     processData(resData){
      ...
      let {newDate,newYear}= this.filterTime(resData)
       this.newDate = newDate;
       this.newYear = newYear;
      /*sortByYear是根据年份划分的数组*/
      let sortByYear = this.filterByYear(resData,newYear)
       this.sortByYear = sortByYear;
      /*sortByDate是根据日期划分的数组*/
      let sortByDate = this.filterByDate(sortByYear,newDate)
...
//       let yearMark = this.hoverShowYearMark(st)
      ...
      /*1.初步过滤时间数组*/
        ...
        for(let i=0;i<arr.length;i++){
          ...
          /*newDate就是新的纯日期数组*/
          newDate = Array.from(new Set(sortDate))
        }
        /*处理处日期数组，和年份数组*/
        for(let j=0;j<sortDate.length;j++){
          /*过滤日期数组（去重排序),此方法修改了原数组，弃用*/
         /* for (let k = 0; k < sortDate.length; k++) {
          ...
          /*截取获得纯年份数组(去重，排序）*/
          sortYear.push(newDate[j].substring(0,4))
          newYear = Array.from(new Set(sortYear))
          /*此方法修改了原数组，弃用*/
...
//        this.$refs[index][0].cssStyle='background-color:#eee;color:#333';
        /*this.$refs[index][0].style={
          backgroundColor:'#eee',
          color:'#333'
        }*/
        this.$refs[index][0].style.backgroundColor = '#eee'
        this.$refs[index][0].style.color = '#333'
        this.$refs[index][0].style.border = '0.04rem solid #ddd'
      ...
      leaveShowDateMark(st,index){
        ...
        this.$refs[index][0].style.backgroundColor = '#404040'
        this.$refs[index][0].style.color = '#fff'
        this.$refs[index][0].style.border = '0.04rem solid #ddd'
      },
      clickShow(st,index){
        this.$refs[index][0].style.backgroundColor = '#666'
        this.$refs[index][0].style.color = '#fff'
        this.$refs[index][0].style.border = '0.07rem solid #ddd'
      ...
      /*按照年份分类，形如：{2014:[],2015:[],2016:[],2017:[]}*/
      testYear(sbd){
        function getObj(sbd) {
          const obj = {};
          sbd.forEach(item =>{
            const year = item[0].sortTime.substring(0,4);
            if(obj[year]){
              obj[year].push(item[0])
            }else {
              obj[year] = [item[0]]
            }
          });
        ...
        let yearGroup = getObj(sbd)
        return yearGroup
      },
      _addClass(index){
//        debugger
        let leafs = document.getElementsByClassName('each-leafs')
       let cc =  this.$refs[index]
        this.$nextTick(()=>{
          console.log(leafs[0],"eah_____________________________________________________________")
          console.log(cc,"cc_____________________________________--")
        })

        let bb = $('.each-leafs')
        console.log(bb,"bb_______________________________________________")
        let leafArr = []
//        for (var i = 0; i < leafs.length; i++) {
//          leafArr.push(leafs[i])
//        }
//        console.log(leafs,"$('.each-leafs')________________________________________--")
        /*$('.each-leafs')*/
        return 'aa'
      },
      _initLefts(){
